...
bool RtpPacket::ParseBuffer(const uint8_t* buffer, size_t size) {
  ...
  const uint8_t version = buffer[0] >> 6;           // 前2位是版本号
  if (version != kRtpVersion) {
    return false;
  }
  const bool has_padding = (buffer[0] & 0x20) != 0;   // 获取第3位数值大小
  const bool has_extension = (buffer[0] & 0x10) != 0; // 获取第4位数值大小，是否存在RTP扩展头
  const uint8_t number_of_crcs = buffer[0] & 0x0f;    // 第1个字节后4位数值大小
  marker_ = (buffer[1] & 0x80) != 0;                  // 第2个字节的第1位 
  payload_type_ = buffer[1] & 0x7f;                   // 第2个字节的后7位
  ...
  payload_offset_ = kFixedHeaderSize + number_of_crcs * 4;  // payload的起始位置

  if (has_padding) {
    padding_size_ = buffer[size - 1];                       // 存在padding的时候,最后一个字节是padding数据的大小  
    if (padding_size_ == 0) {
  ...
  if (has_extension) {
    ...
    |                             ....                              |
    */
    size_t extension_offset = payload_offset_ + 4;    // 存在扩展头的情况，获取扩展头的其实位置
