...
bool ParseStapAStartOffsets(const uint8_t* nalu_ptr,
  ...
  while (length_remaining > 0) {
    ...
    if (nalu_size > length_remaining)
      return false;
    nalu_ptr += nalu_size;                        // nalu_ptr移动到下一个NALU开始位置
    length_remaining -= nalu_size;

    offsets->push_back(offset + kStapAHeaderSize); // 保持数据开始的位置(加上三个字节kNalHeaderSize + kLengthFieldSize)
    offset += kLengthFieldSize + nalu_size;        // offset移动到下一个NALU开始位置
  }
  return true;
}
...
size_t RtpPacketizerH264::SetPayloadData(
  ...
  for (int i = 0; i < fragmentation->fragmentationVectorSize; ++i) {
    ...
    H264::NaluType nalu_type = H264::ParseNaluType(buffer[0]); // 第一个字节的后面5个位
    if (nalu_type == H264::NaluType::kSps) {
      ...
      rtc::Optional<SpsParser::SpsState> sps;  // 获取了并没有使用
    ...
    switch (packetization_mode_) {
      ...
      case H264PacketizationMode::NonInterleaved:
        ...
        if (i + 1 == input_fragments_.size()) {
          // Pretend that last fragment is larger instead of making last packet smaller.
          // 最后一个包的长度
...
void RtpPacketizerH264::PacketizeFuA(size_t fragment_index) {
  ...
  // Integer divisions with rounding up. Minimal number of packets to fit all payload and virtual payload.
  // 将会打包成多个Packet
...
size_t RtpPacketizerH264::PacketizeStapA(size_t fragment_index) {
  ...
  packets_.back().last_fragment = true;  // 标记为最后一个包
  return fragment_index;  // 多个Fragment打包到Packet
}
...
void RtpPacketizerH264::NextAggregatePacket(RtpPacketToSend* rtp_packet,
  ...
  // payload设置STAP-A NALU header(24).
...
bool RtpDepacketizerH264::Parse(ParsedPayload* parsed_payload,
  ...
  if (nal_type == H264::NaluType::kFuA) {
      ...
      if (!ParseFuaNalu(parsed_payload, payload_data))         // FU-A 单个NAL单元到多个RTP包
      return false;
  } else {
    // We handle STAP-A and single NALU's the same way here. The jitter buffer
    // will depacketize the STAP-A into NAL units later.
    // TODO(sprang): Parse STAP-A offsets here and store in fragmentation vec.
    if (!ProcessStapAOrSingleNalu(parsed_payload, payload_data))  // 单时间聚合包类型A 或者 单个NAL单元包
      return false;
  }
