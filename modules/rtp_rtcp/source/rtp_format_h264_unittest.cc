...
TEST_P(RtpPacketizerH264ModeTest, TestSingleNalu) {
  ...
  RTPFragmentationHeader fragmentation;
  fragmentation.VerifyAndAllocateFragmentationHeader(1);  // 分片的长度为1
  fragmentation.fragmentationOffset[0] = 0;               // 数据的位置索引为0
  fragmentation.fragmentationLength[0] = sizeof(frame);   // 大小为 sizeof(frame) 即 2
  std::unique_ptr<RtpPacketizer> packetizer(
