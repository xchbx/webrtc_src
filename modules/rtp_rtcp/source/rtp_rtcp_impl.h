...
// ModuleRtpRtcpImpl控制模块，是个Module，自己能够独立处理
class ModuleRtpRtcpImpl : public RtpRtcp, public RTCPReceiver::ModuleRtpRtcp {
 ...
 private:
  ...
  std::unique_ptr<RTPSender> rtp_sender_;           // rtp数据发送

  RTCPSender rtcp_sender_;                          // rtcp数据发送
  RTCPReceiver rtcp_receiver_;                      // rtcp数据接收
  ...
  RemoteBitrateEstimator* remote_bitrate_;          // 对端网速探测
