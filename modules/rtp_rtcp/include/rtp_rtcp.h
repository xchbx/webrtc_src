...
class RtpRtcp : public Module, public RtcpFeedbackSenderInterface {
 public:
  struct Configuration {
    ...
    // 数据接收反馈，RtcpReportBlocks
    ReceiveStatisticsProvider* receive_statistics = nullptr;

    // Transport object that will be called when packets are ready to be sent out on the network.
    // 用于将数据发送到网络
    ...
    // Called when we receive a changed estimate from the receiver of out stream.
    ...
    // Estimates the bandwidth available for a set of streams from the same client.
    ...
    TransportSequenceNumberAllocator* transport_sequence_number_allocator = nullptr;
