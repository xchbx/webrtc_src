...
class AudioTransport {
 public:
  // 回调获取Recording音频数据
  virtual int32_t RecordedDataIsAvailable(const void* audioSamples,       // (数据大小) nSamples * nBytesPerSample * nChannels
                                          const size_t nSamples,          // 480 (10毫秒一个包) nSamples * 100 = samplesPerSec
                                          const size_t nBytesPerSample,   // 采样位数(2bytes = 16bits) nBytesPerSample = 2* nChannels
                                          const size_t nChannels,         // 声道数(1或者2)
                                          const uint32_t samplesPerSec,   // 采样频率(> 8KHz) 
                                          const uint32_t totalDelayMS,    // 整体延时
