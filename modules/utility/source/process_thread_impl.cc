...
void ProcessThreadImpl::Start() {
  ...
  // 模块关联到当前的ProcessThread
  for (ModuleCallback& m : modules_)
    m.module->ProcessThreadAttached(this);
 
  // 创建启动线程，运行ProcessThreadImpl::Run函数(ProcessThreadImpl::Process)
  thread_.reset(
...
bool ProcessThreadImpl::Process() {
...

  {
    ...
    for (ModuleCallback& m : modules_) {
      ...
      // 调用该模块的TimeUntilNextProcess()函数)。
      // 如果时刻是当前时刻，则调用模块的Process()处理函数，并更新下次调用时刻。
      // 需要注意，不同模块的执行频率不一样，线程在本轮调用末尾的等待时间和本线程下所有模块的最近下次调用时刻相关。
