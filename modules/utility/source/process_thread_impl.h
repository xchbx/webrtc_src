...
class ProcessThreadImpl : public ProcessThread {
 ...
 private:
  ...
  const std::unique_ptr<EventWrapper> wake_up_;       // 用来唤醒处于等待状态的线程
  // TODO(pbos): Remove unique_ptr and stop recreating the thread.
  std::unique_ptr<rtc::PlatformThread> thread_;       // 平台相关的线程实现,如POSIX线程

  ModuleList modules_;                                // 注册在本线程下的模块集合
  std::queue<rtc::QueuedTask*> queue_;                // 邮递给本线程的任务集合
  bool stop_;
  const char* thread_name_;                           // 线程名字
};
