...
struct WebRtcRTPHeader {
  RTPHeader header;         // RTP头
  FrameType frameType;      // 帧类型,kEmptyFrame、kVideoFrameKey、kVideoFrameDelta等
  RTPTypeHeader type;       // 音频或者视频
...
class RTPFragmentationHeader {
 public:
  ...
  size_t* fragmentationOffset;       // Offset of pointer to data for each fragmentation
  size_t* fragmentationLength;       // Data size for each fragmentation
  uint16_t* fragmentationTimeDiff;   // Timestamp difference relative "now" for each fragmentation
