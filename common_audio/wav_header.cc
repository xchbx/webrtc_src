...
struct FmtSubchunk {
  ChunkHeader header;
  uint16_t AudioFormat;   // 编码格式代码 PCM 0x0001 G.711 a-law 0x0006
  uint16_t NumChannels;   // 声道个数
  uint32_t SampleRate;    // 采样频率
  uint32_t ByteRate;      // 数据传输速率
...
struct WavHeader {
  struct {
    ChunkHeader header;   // 大写字符串"RIFF",标明该文件为有效的 RIFF 格式文档。
    uint32_t Format;      // 文件格式类型"WAVE"
  } riff;                 
  FmtSubchunk fmt;        // fmt 块
  // 缺失 fact 块(压缩编码格式要含有该块)  
  struct {
    ChunkHeader header;
  } data;                 // data 块
};
...
// num_channels * sample_rate * bytes_per_sample
//  声道数 × 采样频率 × 每样本的数据位数/8
...
void WriteWavHeader(uint8_t* buf,
                    size_t num_channels,        // 声道
                    int sample_rate,
                    WavFormat format,         
                    size_t bytes_per_sample,    // 采样位数
                    size_t num_samples) {       // 采样大小
  RTC_CHECK(CheckWavParameters(num_channels, sample_rate, format,
                               bytes_per_sample, num_samples));

  WavHeader header;
  // payload数据大小                采样位数            采样大小
