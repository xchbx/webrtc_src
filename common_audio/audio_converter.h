...
// audio_frame_operations.h 支持的操作会多一点

// Format conversion (remixing and resampling) for audio. 
// Only simple remixing conversions are supported: downmix to mono (i.e. |dst_channels| == 1) or
...
class AudioConverter {
 public:
  ...
  static std::unique_ptr<AudioConverter> Create(size_t src_channels,    // 原数据通道数
                                                size_t src_frames,      // 原数据帧大小
                                                size_t dst_channels,    // 目标数据通道数
                                                size_t dst_frames);     // 目标数据帧大小
