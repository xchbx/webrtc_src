...
class AudioEncoder {
 public:
  ...
  // Returns the number of 10 ms frames the encoder will put in the next packet.
  // This value may only change when Encode() outputs a packet; i.e.,
  // the encoder may vary the number of 10 ms frames from packet to packet, but
  // it must decide the length of the next packet no later than when outputting
  // the preceding packet.
  virtual size_t Num10MsFramesInNextPacket() const = 0;

  // Returns the maximum value that can be returned by Num10MsFramesInNextPacket().
  // 返回Num10MsFramesInNextPacket函数可以返回的最大值
  virtual size_t Max10MsFramesInAPacket() const = 0;

  // Returns the current target bitrate in bits/s. The value -1 means that the
  // codec adapts the target automatically, and a current target cannot be provided.
  ...
  // supported).
  // Discontinuous Transmission不连续,发送在没有语音信号传输时停止发射无线信号，使干扰降低，来提高系统的效率。
  virtual bool SetDtx(bool enable);

  // Returns the status of codec-internal DTX. The default implementation always returns false.
 ...
 protected:
  // Subclasses implement this to perform the actual encoding. Called by Encode().
