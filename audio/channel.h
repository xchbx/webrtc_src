...
// http://www.cnblogs.com/fangkm/p/4401075.html

// Channel是对一路音频数据及其处理过程的抽象，是VoiceEngineImpl中最重要的底层实现类，
// 其继承并实现RtpData、RtpFeedback、Transport、AudioPacketizationCallback、OverheadObserver等多个接口
// 分别负责音频数据编码后回掉、发送到网络、接收后存储到NetEq缓冲区、播放前混音等一些列重要操作。
class Channel
    : public RtpData,                     // OnReceivedPayloadData(...)
      public RtpFeedback,                 // OnIncomingSSRCChanged(...) 
      public Transport,                   // SendRtp(...) && SendRtcp(...)
      public AudioPacketizationCallback,  // SendData(...) : receive encoded packets from the ACM
      public OverheadObserver {           // OnOverheadChanged(...)
 public:
  friend class VoERtcpObserver;

  enum { KNumSocketThreads = 1 };
  enum { KNumberOfSocketBuffers = 8 };
  // Used for send streams.  用于发送数据的Channel的构造函数
  Channel(rtc::TaskQueue* encoder_queue,          // 编码队列
          ProcessThread* module_process_thread,   // 模块线程
          AudioDeviceModule* audio_device_module, // ADM
          RtcpRttStats* rtcp_rtt_stats);

  // Used for receive streams. 用于接收数据的Channel的构造函数
  ...
  // DTMF(Dual Tone Multi Frequency). 双音多频  https://baike.baidu.com/item/DTMF/3106215
 ...
 private:
  ...
  // Called on the encoder task queue when a new input audio frame is ready for encoding.
  ...
  std::unique_ptr<RtpRtcp> _rtpRtcpModule;                  //  rtpRtcp模块
  std::unique_ptr<AudioCodingModule> audio_coding_;         // 音频数据编解码模块
  ...
  AudioDeviceModule* _audioDeviceModulePtr;                           // AudioDeviceModule模块
  ...
  rtc::TaskQueue* encoder_queue_ = nullptr;     // 编码任务队列
};
