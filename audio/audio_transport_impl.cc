...
void InitializeCaptureFrame(int input_sample_rate,      // 输入音频的采样率
                            int send_sample_rate_hz,    // 输出音频的采样率
                            size_t input_num_channels,  // 输入音频的通道数量
                            size_t send_num_channels,   // 输出音频的通道数量
  ...
  for (int native_rate_hz : AudioProcessing::kNativeSampleRatesHz) {
    // audio_frame->sample_rate_hz_ 设置为AudioProcessing::kNativeSampleRatesHz 中最小的大于min_processing_rate_hz的数值
...
// 设置接受端需要的采样频率通道和AudioSendStream对象
