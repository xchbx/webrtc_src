...
bool Conductor::CreatePeerConnection(bool dtls) {
  ...
  // 获取默认的ICE Server，默认为 stun:stun.l.google.com:19302
...
void Conductor::StartLogin(const std::string& server, int port) {
  if (client_->is_connected())
    return;
  server_ = server;
  // 命令PeerConnectionClient::Connect执行连接操作
...
void Conductor::ConnectToPeer(int peer_id) {
  ...
  if (InitializePeerConnection()) {
    peer_id_ = peer_id;
	  // 当Conductor创建Offer成功后, 回调Conductor::OnSuccess被调用
    peer_connection_->CreateOffer(this, NULL);
  } else {
...
std::unique_ptr<cricket::VideoCapturer> Conductor::OpenVideoCaptureDevice() {
  ...
  // 创建cricket::VideoCapturer对象
  cricket::WebRtcVideoDeviceCapturerFactory factory;
  std::unique_ptr<cricket::VideoCapturer> capturer; // 具体实现为WebRtcVideoCapturer
...
void Conductor::AddStreams() {
  ...
  // MediaStreamInterfacet添加音频视频
  stream->AddTrack(audio_track);
  if (video_track)
    stream->AddTrack(video_track);

  // peer_connection_添加Stream
