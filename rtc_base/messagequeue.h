...
class MessageQueue {
 public:
  ...
  // rtc::MessageQueue::PostDelayed：发送一个延迟消息（从当前时间计算延迟处理时间）
  // cmsDelay：延迟毫秒数
  // phandler：消息处理器（rtc::MessageHandler的子类）
  // id：消息ID
  // pdata：MessageData指针
  ...
  virtual void PostAt(const Location& posted_from,
                      ...
                      uint32_t id = 0,
                      MessageData* pdata = nullptr);
  // 通过指定talk_base::MessageHandler和消息ID删除消息
 ...
 protected:
  ...
  bool fPeekKeep_;
  Message msgPeek_;
  // 消息队列，队列内的消息按照先进先出的原则立即执行
  MessageList msgq_ RTC_GUARDED_BY(crit_);
  // 延迟消息队列，队列内的消息按照指定的时间延迟执行
 ...
 private:
  ...
  // 协同工作的rtc::SocketServer
  SocketServer* const ss_;
  // Used if SocketServer ownership lies with |this|.
  // 默认的rtc::SocketServer。
  // 如果在构造的时候提供rtc::SocketServer，那么消息队列就使用用户提供的；
  // 如果没有提供，消息队列就初始化一个rtc::PhysicalSocketServer并保存在default_ss_上，然后赋值给ss_。
