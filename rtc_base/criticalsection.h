...
// everywhere.
// rtc::CriticalSection主要实现了跨平台实现临界区的功能。
...
// 利用构造函数加锁rtc::CriticalSection；并利用析构函数在退出代码块的时候解锁rtc::CriticalSection。
...
// 利用构造函数尝试加锁rtc::CriticalSection；并利用析构函数在退出代码块的时候解锁rtc::CriticalSection。
class TryCritScope {
 public:
