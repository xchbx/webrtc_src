...
class ThreadManager {
 public:
  ...
  // 获取代表当前线程的rtc::Thread实例。
  // 这就要求在创建系统线程的同时能够立即将它自动封装成rtc::Thread并加以合理地保存。
  ...
  // 将当前线程封装成一个rtc::Thread对象，并存储在TLS中。
...
#if defined(WEBRTC_POSIX)
  // TLS能够在线程空间内保存一些数据，所有的线程通过它只能访问到自己线程空间的变量，各个线程之间互不影响。
  pthread_key_t key_;      // key_：系统TLS（Thread LocalStorage）的key
#endif
...
// 定制rtc::Threaad::Run函数的行为可以继承rtc::Runnable类，并将实例传递给rtc::Thread::Start函数
class Runnable {
 public:
  virtual ~Runnable() {}
...
// Thread主要功能是封装线程，为线程提供消息队列和多路信号分离器的功能。
class RTC_LOCKABLE Thread : public MessageQueue {
 public:
  ...
  // 获取封装当前线程的rtc::Thread对象，该功能由rtc::ThreadManager提供
  ...
  // 停止消息循环，并调用rtc::Thread::Join等待线程结束
  virtual void Stop();
  ...
  // 向一个线程发送消息，并等待消息完成，被请求线程可以是当前线程
  virtual void Send(const Location& posted_from,
                    MessageHandler* phandler,   // 消息处理器
                    uint32_t id = 0,
  ...
  // From MessageQueue
  // 继承自talk_base::MessageQueue::Clear函数，为了保持父类该函数的语义需要同时删除sendlist_列表中的消息
  ...
  // 封装/解除封装当前线程，该功能由rtc::ThreadManager提供
  bool WrapCurrent();
  void UnwrapCurrent();
 ...
 private:
  ...
  std::list<_SendMessage> sendlist_;        // 通过rtc::Thread::Send函数请求处理的消息列表
  std::string name_;
...
#if defined(WEBRTC_POSIX)
  pthread_t thread_ = 0;                    // 保存系统的线程句柄/pthread_t对象
#endif
  ...
  bool owned_ = true;         // 标示rtc::Thread对象是否是线程的所有者
